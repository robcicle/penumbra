...
namespace penumbra::Math
{
	bool DecomposeTransform(const glm::mat4& matTransform, glm::vec3& position, glm::vec3& rotation, glm::vec3& scale)
	{
		...
		glm::mat4 LocalMatrix(matTransform);
		...
		LocalMatrix[3] = glm::float4(0, 0, 0, LocalMatrix[3].w);
	...
	{
		return glm::inverse(matParentTransform) * matWorldTransform;
	}
}